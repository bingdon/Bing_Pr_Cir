...
public class UpdataAppUtlity {
	...
	public static void upDateApp(Context context) {
		UpdataAppUtlity.context = context;
		// HealthHttpClient.doHttpUpdateApp(handler);
		MyHttpClient.listVersion(handler);
	}

	private static String getLoVersion() {

		PackageManager packageManager = UpdataAppUtlity.context
				.getPackageManager();
		try {
			PackageInfo packageInfo = packageManager.getPackageInfo(
					UpdataAppUtlity.context.getPackageName(), 0);
	...
	private static void downDialog(final String url, String info) {
		new AlertDialog.Builder(UpdataAppUtlity.context)
				.setTitle(
						UpdataAppUtlity.context.getResources().getString(
								R.string.downnotice))
				.setMessage(info)
	...
	private static void downLoad(String url) {
		DownloadManager downloadManager = (DownloadManager) UpdataAppUtlity.context
		...
		try {
			...
			request.setTitle(UpdataAppUtlity.context.getResources().getString(
					R.string.app_name));
